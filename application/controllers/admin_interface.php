...
class Admin_interface extends MY_Controller {
    ...
    public function subCategories() {
        ...
        if ($this->input->get('category') !== FALSE && is_numeric($this->input->get('category'))):
            $categories = $this->categories->getWhere(NULL, array('sub_category' => 1,
                'parent' => $this->input->get('category')), TRUE);
        else:
    ...
    public function productsList() {
        ...
        if ($this->input->get('watermark') == 1):
            $this->load->model('products_resources');
            foreach ($pagevar['products'] as $product):
                if($images = $this->products_resources->getWhere(NULL, array('product' => $product['id']), TRUE)):
                    foreach($images as $image):
                        if(file_exists(getcwd().'/'.$image['resource']) && file_exists(getcwd().'/'.$image['thumbnail'])):
                            $this->watermark(getcwd().'/'.$image['resource'], getcwd().'/img/watermark/main.png');
                            $this->watermark(getcwd().'/'.$image['thumbnail'], getcwd().'/img/watermark/main.png');
                        endif;
                    endforeach;
                endif;
            endforeach;
